    ...
    <div className="w-full bg-gray-900 text-gray-200">
      ...
      <header className="bg-gray-800 p-4 border-b border-blue-500/30 flex items-center justify-between fixed w-full">
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          <NavLink><Calendar size={18} />Appointments</NavLink>
          <NavLink><FileText size={18} />Records</NavLink>
          <NavLink color="red"><LogOut size={18} />Logout</NavLink>
        </div>
      ...
      {/* Main Content */}
      <main className="grid md:grid-cols-3 gap-4 p-2  bg-amber-300  pt-24">
        {/* Left Column - Appointment Selection */}
        <div className="md:col-span-1 h-full bg-gray-800/80 rounded-lg border border-gray-700 p-4 relative">
          {/* <div className="mb-6 ">
          ...
          <div className="mb-4 bg-orange-300">
            {/* <h3 className="text-gray-300 mb-2 flex items-center">
              <Clock size={16} className="mr-2 text-blue-400" />
              Time Slots
            </h3>
            <div className="h-full grid grid-cols-1 gap-2 max-h-64 overflow-y-auto pr-1">
        ...
        {/* Middle Column - Patient Info and Prescriptions */}
        <div className="md:col-span-2">
          ...
          <div className="bg-gray-800/80 rounded-lg border border-gray-700 p-4 mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-xl font-medium text-white truncate">{patientInfo.name}</h2>
              ...
              <div className="text-right">
                <div className="text-lg font-medium text-blue-400">{patientInfo.patientId}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800/80 rounded-lg border border-gray-700 p-4">
            ...
            <div className="mb-4">
              ...
              <div className="bg-gray-700 rounded-lg p-2 min-h-16 border border-gray-600">
                {selectedMedicines.length > 0 ? (
                  <ul className="space-y-2">
                    {selectedMedicines.map(medicine => (
                      <li key={medicine.id} className="flex items-center justify-between bg-gray-800 p-2 rounded">
                        <div>
                          <span className="text-blue-400">{medicine.name}</span>
                          <span className="text-gray-400 text-sm ml-2">× {medicine.quantity}</span>
                        </div>
                        <button 
...
const NavLink: React.FC<NavLinkProps> = ({ children, active, color }) => {
  const baseClasses = "flex items-center space-x-2 px-4 py-3 rounded-md transition-colors";
